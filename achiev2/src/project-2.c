...
int main (int argc,char*argv[])
{
  ...
  int n = players;//BEGIN init_queue
  ...
  for (int i = 0; i < n; i++) {  // on initialise la queue   
    struct teddy ted = {i,  // le numero du teddy 
			...
			NULL //la transaction favorite du teddy, ce champ n'est utilise que si le teddy est stratege de niveau 3, 4  ou 5 
    };
    tab[i] = ted;
  ...
  while (global_time < turns) {
    ...
    if ( ! utility__transac_cant_make(active_ted, transac, 1)) {
      active_ted->w = utility__wallet_after_transac(active_ted, transac,1); // le teddy effectue la transaction 
