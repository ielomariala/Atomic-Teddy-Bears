...
// Retourne soit la transaction favoris du teddy s'il peut la faire, et si la transaction qu'a choisit 'ted' par "strategy1" est moins optimale. Sinon la transaction favoris est mise à jour, et cette fonction retourne effectivement strategy1(ted) 
const struct transac* strategy__strategy3(struct teddy *ted);

// Retourne soit la transaction favoris du teddy s'il peut la faire, et si la transaction qu'a choisit 'ted' par "strategy2" est moins optimale. Sinon la transaction favoris est mise à jour, et cette fonction retourne effectivement recursive_strategy(ted) 
const struct transac* strategy__strategy4(struct teddy *ted);

// Le teddy découvre toutes les places d'échange et choisit parfaitement sa transaction favoris, ensuite il la fait s'il peut, sinon il fait la transaction avec le maximum de profit dans sa place d'échange actuelle
