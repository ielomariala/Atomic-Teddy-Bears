...
int main (int argc,char* argv[])
{
  ...
  int n = players;
  const struct stockex* stock = starting_stockex();
  ...
  while (global_time < turns) {
    ...
    size_t j = random (stockex__num_transactions(stock));  // il choisi une transaction aleatoirement
    const struct transac* transac = stockex__transaction(stock, j);
    ...
    queue__push(&Q , active_ted->time_played , active_ted);  // on remet le teddy dans la queue, a la bonne place
  }
