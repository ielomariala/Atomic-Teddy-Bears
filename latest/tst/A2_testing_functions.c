...
int strategy_tests( int test ) {
 ...
 const struct stockex stock6 = {
  ...
  int players = 6, seed = 0, turns = 50;
  int  gagne0 = 0, gagne1 = 0, gagne2 = 0, gagne3 = 0, gagne4 = 0, gagne5 = 0 ;
  generate_seed(seed);
  int nbre = players;  
  ...
  case 4: // On lance une boucle de jeu 100 fois, notre intention est de calculer le pourcentage de victoire de chaque stratégie, mais le graphe de places d'échange non aléatoire fait en sorte qu'un teddy gagne 100 fois meme si les valeurs des ressources changent au long du jeu
    for (int k=0; k < 100; k++) {
	...
	if ( ! utility__transac_cant_make(active_ted, transac, 1) ) {
	  active_ted->w = utility__wallet_after_transac(active_ted->w,transac,1);
	...
	Q.tab_queue[k]->value = wallet__value(Q.tab_queue[k]->w);
      ...
      if (Q.tab_queue[nbre-1]->num == 3) { gagne3++;}
      if (Q.tab_queue[nbre-1]->num == 4) { gagne4++;}
      if (Q.tab_queue[nbre-1]->num == 5) { gagne5++;} 
    }
    if ( ! gagne0 ){
      return 1;
    }
