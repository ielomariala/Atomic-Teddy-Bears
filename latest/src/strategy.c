...
const struct transac* strategy__transac_add(const struct transac* t, const struct transac* s)
{
  for (int i=0 ; i<MAX_GOOD; i++)
    {
      transac__in_wallet(t).data[i] += transac__in_wallet(s).data[i];
      transac__out_wallet(t).data[i] += transac__out_wallet(s).data[i];
    }
  return t;
}
...
const struct transac* strategy__strategy2(struct teddy* ted)
{
  ...
  const struct stockex* stock = ted->current_stock;
  size_t i = 0, n = stockex__num_transactions(stock);
  int profitab_stock[MAX_TRANSAC][MAX_TRANSAC];
  int profitab[MAX_TRANSAC];
  for (i = 0 ; i < n ; i++){
