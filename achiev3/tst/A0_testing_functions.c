...
int stockex_tests()
{
  const struct stockex* p = NULL;
  const struct transac s0 = {
			     {{10,1,0}} ,  //acheté par le teddy
			     {{0,0,1}} ,  //vendu
			     p ,  //place d'echange de la transaction  (NULL pour ce test)
			     p  //place d'echange debloquee (NULL pour ce test)
  };
  const struct transac s1 = {
			     {{0,0,1}} ,  //acheté par le teddy
			     {{20,0,0}} ,  //vendu
			     p ,  //place d'echange dela transaction  (NULL pour ce test)
			     p  //place d'echange debloquee (NULL pour ce test)
  };
  const struct transac s2 = {
			     {{0,2,1}} ,  //acheté par le teddy
			     {{20,0,0}},  //vendu
			     p ,  //place d'echange de la transaction (NULL pour ce test)
			     p  //place d'echange debloquee (NULL pour ce test)
  };
  ...
  if (w_achete.data[0] == 0 && w_achete.data[1] == 0 && w_achete.data[2] == 1 ){  //transac_in_wallet
    ...
    if (w_vendu.data[0] == 10 && w_vendu.data[1] == 1 && w_vendu.data[2] == 0 ){  //transac_out_wallet
...
//playing with teddy and queue structures
int queue_tests()
{
  const struct stockex* p = NULL;
  const struct transac s0 = {
  ...
  struct teddy* ted_prioritaire = queue__top(&q);  //recupperer le teddy prioritaire
  if (ted_prioritaire->time_played == 100) { 
    k++;
  }
  else {
  ...
  struct teddy* ted_prioritaire2 = queue__top(&q);
  if (ted_prioritaire2->time_played == 1) {
  ...
  queue__push(&q2, ted.time_played, &ted);   //ajouter un teddy à une queue pleine 
