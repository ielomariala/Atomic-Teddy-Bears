...
{     
  if (h->n < MAX_TED) { // on ne remplis pas une queue pleine    
    int len = h->n;// `i` est la longueur de queue avant le push, et l'indice du dernier teddy de la queue après
    int j = len-1;
    ...
    while ( j >= 0  && (h->tab_queue[j])->time_played < priority ) { 
      ...
      j--;
    }  // tant que c'est pas la bonne place de l'ours 't', on décale l'ours courant et on décrémente 'j'. Si j<0, l'ours 't' est celui qui a joué le plus, il rentre dans la position d'indice 0
    h->tab_queue[j+1] = t; 
  }
  else { printf("Attention, cette queue est pleine.\n");}
}
